...
namespace egocylindrical
{
    ...
    bool NormalPointCloudFloorOnlyGenerator::init()
    {
        ...
        ros::SubscriberStatusCallback info_cb = boost::bind(&NormalPointCloudFloorOnlyGenerator::ssCB, this);
        {
            Lock lock(connect_mutex_);
            pc_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("normals_floor_only", 2, info_cb, info_cb);
        }
    ...
    void NormalPointCloudFloorOnlyGenerator::ecPointsCB(const egocylindrical::EgoCylinderPoints::ConstPtr& ec_msg)
    {
        ...
        if(pc_pub_.getNumSubscribers()>0)
        {
          ...
          sensor_msgs::PointCloud2::ConstPtr msg = utils::generate_normal_point_cloud_floor_only(ec_pts);
